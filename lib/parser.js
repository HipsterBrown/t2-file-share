    ...
    if (!this.previewIDFOffset) {
      if (this.previewType === 'full') {
        ...
        try {
          ...
          const dirLength = data.readUIntLE(firstIDFOffset, 2);
          ...
          try {
            ...
            console.error('Issue getting tags for thumbnail: ', error);
          }
        } catch (error) {
          console.error('Issue getting first directory tags: ', error);
        }
      }
    }
...
function getTags(imageBuffer, offset) {
  ...
  for (let i = 0; i < dirLength; i++) {
    ...
    try {
      ...
      console.error('Issue getting exif directory: ', error);
    }
    ...
    try {
      ...
      console.error('Issue getting maker directory: ', error);
    }
  }
